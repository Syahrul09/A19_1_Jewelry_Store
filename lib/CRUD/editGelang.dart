...
class Edit_Gelang extends StatefulWidget {
  final String id;
  final String nama;
  final String jenis;
  ...
  const Edit_Gelang(this.id, this.nama, this.jenis, this.kategori, this.stok,
      this.harga, this.desc,
      {Key? key})
      : super(key: key);
  @override
  State<Edit_Gelang> createState() => _Edit_GelangState();
}

class _Edit_GelangState extends State<Edit_Gelang> {
  ...
  Future unggahFile(ambilFile, id) async {
    ...
    Reference ref = storage.ref().child("gambar_gelang").child(fileName);
    UploadTask uploadTask = ref.putFile(imageFile);
    TaskSnapshot snapshot = await uploadTask.whenComplete(() => null);
    url = await snapshot.ref.getDownloadURL();

    produk_gelang.doc(id).update({
  ...
  return showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          content: Text(pesan),
          actions: [
            TextButton(onPressed: () => Get.to(AdminPage()), child: Text("OK"))
          ],
        );
      });
}
